...
class RWKV7Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif len(value_dim) != num_hidden_layers:
            raise ValueError("`value_dim` must have the same length as `num_hidden_layers`.")

        for dim in value_dim:
            if dim % num_heads != 0:
                raise ValueError("Every `value_dim` entry must be divisible by `num_heads`.")
